...
class MixMatch(models.MultiModel):
	...
	def guess_label(self, y, classifier, T, **kwargs):
		del kwargs 
		logits_y = [tf.reduce_mean(classifier(yi, training=True), axis=0) for yi in y]
		#logits_y = [classifier(yi, training=True) for yi in y]
		# shape: (B, nu * num_final_layers, num_classes)	
	...
	def model(self, batch, lr, wd, ema, beta, w_match, warmup_kimg=1024, nu=2, mixmode='xxy.yxy', **kwargs):
		num_final_layers = kwargs['num_final_layers']
		...
		if nu == 1:
			guess = self.guess_label([y], classifier, T=0.5, **kwargs)
		else:
		...
		if nu == 1:
			xy, labels_xy = augment([x_in] + [y], [lx] + [ly], [beta, beta])
		else:
		...
		logits = [tf.reduce_mean(classifier(batches[0], training=True), axis=0)]
		post_ops = [v for v in tf.get_collection(tf.GraphKeys.UPDATE_OPS) if v not in skip_ops]

		temp = []
		logits_y_list = [None] * num_final_layers

		for batchi in batches[1:]:
			temp.append(classifier(batchi, training=True))
		for i in range(num_final_layers):
			logits_ = logits + [temp[n][i] for n in range(nu)]
			logits_ = layers.interleave(logits_, batch)
			logits_y_list[i] = tf.concat(logits_[1:], 0)
		logits_x = logits_[0]

		loss_xe = tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels_x, logits=logits_x/0.5)
		loss_xe = tf.reduce_mean(loss_xe)
		loss_l2u = tf.add_n([tf.square(labels_y - tf.nn.softmax(logits_y)) for logits_y in logits_y_list])
...
if __name__ == '__main__':
	...
	flags.DEFINE_integer('num_final_layers', 1, 'Number of final layers.')
	FLAGS.set_default('dataset', 'cifar10.3@250-5000')
	FLAGS.set_default('batch', 64)
	FLAGS.set_default('lr', 0.002)
	FLAGS.set_default('train_kimg', 1 << 16)
	FLAGS.set_default('arch', 'ensemble')
	app.run(main)
